...
date = 2014-06-18
slug = "become-an-erlang-cowboy-and-tame-the-wild-wild-web-part-i"
...
> **Rust** is a systems programming language that**runs blazingly fast** , **prevents almost all crashes** , and eliminates data races. — <http://www.rust-lang.org/>
...
> **Ruby** is a **dynamic** , open source programming language with a **focus on simplicity and productivity**. It has an **elegant syntax that is natural to read and easy to write**. — [https://www.**ruby** -lang.org/en/](https://www.ruby-lang.org/en/)
...
A really interesting property, helped by single assignment and inmutability, is [referential transparency](https://en.wikipedia.org/wiki/Referential_transparency_%28computer_science%29). In plain english this means that the result of a function is always the same when you provide the same arguments/input. You might think this is a property of most programming languages, but it is not.
...
> Besides addressing a process by using its pid, there are also built in functions (BIFs) for registering a process under a name. The name must be an atom and is automatically unregistered if the process terminates
