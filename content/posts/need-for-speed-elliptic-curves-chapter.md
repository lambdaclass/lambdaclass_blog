+++
title = "Need for speed: Elliptic curves chapter"
date = 2022-08-12
slug = "need-for-speed-elliptic-curves-chapter"

[extra]
feature_image = "/content/images/2025/12/Foundation_of_the_Ottoman_Empire.jpg"
authors = ["LambdaClass"]
...
Some curves can go even faster. If $x^3 + ax + b$ has a solution in $\mathcal{F_p}$, we can work with an equivalent Jacobi quartic $v^2 = a^\prime u^4 + du^2 + 1$, where $a^\prime$ and $d$ depend on the root. We can transform the curve $(u,v)$ to 3-d space $(U,V,W)$ using $u = U/W$ and $v = V / W^2$ and get the equation

$$V^2 = a^\prime U^4 + dU^2 W^2 + W^4$$

If we want to sum $P_3 = P_1+P_2$, in these coordinates we have:

$U_3 = U_1W_1V_2 + U_2W_2V_1$  
$V_3 = ((W_1 W_2)^2 + a^\prime (U_1 U_2)^2)(V_1 V_2 + dU_1 U_2 W_1 W_2) + 2a^\prime U_1 U_2 W_1 W_2 ({U_1}^2 {W_2}^2 + {U_2}^2 {W_1}^2)$  
$W_3 = (W_1 W_2)^2 - a^\prime (U_1 U_2)^2$
