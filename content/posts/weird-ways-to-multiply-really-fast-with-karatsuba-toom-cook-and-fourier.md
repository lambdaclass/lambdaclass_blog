+++
title = "Weird ways to multiply really fast with Karatsuba, Toom–Cook and Fourier"
date = 2023-01-02
...
$w(t)=w_4t4+w_3t3+w_2t^2+w_1t+w_0$  
...
FFT can be extended from complex or real numbers to arbitrary rings, such as integers or polynomials (check our [math survival kit](/math-survival-kit-for-developers/)). In particular, we can use the number theoric transform which specializes the FFT to $\mathbb{Z}/p\mathbb{Z}$, that is, the integers modulo $p$ (a prime number). Here we also have the $n$-roots of unity, given by  
...
$p(x)=p_e(x)+xp_o(x)$.  
For example, if $p=x5+3x4+5x3+2x2+6x+3$, we split it:  
$p(x)=(3x4+2x2+3)+x(x4+5x2+6)$  
We have then:  
$p_e=(3x4+2x2+3)$ and $p_o=(x4+5x2+6)$, where both polynomials are even functions! This way, we easily see that:  
$p(-x)=p_e(x)-xp_o(x)$  
